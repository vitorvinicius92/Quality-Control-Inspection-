...
# ---------------- Config ----------------
DB_URL = "sqlite:///rnc.db"  # SQLite local (Streamlit Cloud friendly)
...
def next_rnc_num_for_date(d:date) -> str:
    """Generate next RNC number for the given date, format YYYY-NNN (per year)."""
    year = d.year
    prefix = f"{year}-"
    # Fetch max existing for that year
    with engine.begin() as conn:
        rows = conn.execute(text("SELECT rnc_num FROM inspecoes WHERE rnc_num LIKE :pfx"), {"pfx": f"{prefix}%"}).fetchall()
    seqs = []
    for (val,) in rows:
        if not val: 
            continue
    ...
    return f"{year}-{next_seq:03d}"  # zero-padded to 3 dígitos
...
# ---------------- UI & Auth ----------------
st.set_page_config(page_title="RNC — v2.5 (Auto RNC Nº)", page_icon="🧭", layout="wide")
st.sidebar.title("RNC — v2.5")
st.sidebar.caption("RNC Nº automático • PEP+descrição • Perfis")
...
if menu == "Nova RNC":
    ...
    with st.form("form_rnc"):
        ...
        with col2:
            # preview do próximo número (readonly)
            preview_num = next_rnc_num_for_date(data_insp)
            st.text_input("RNC Nº (gerado automaticamente)", value=preview_num, disabled=True)
        ...
        if submitted:
            # garante numeração no momento do salvar (evita conflito se alguém criou no mesmo minuto)
...
# -------- Exportar --------
    ...
    if df.empty:
        ...
        st.download_button("Baixar CSV", data=csv_bytes, file_name="rnc_export_v2_5.csv", mime="text/csv")
