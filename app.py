...
EMAIL_TO = [e.strip() for e in os.getenv("EMAIL_TO", "").split(",") if e.strip()]  # lista

APP_BASE_URL = os.getenv("APP_BASE_URL", "")  # opcional, link no e-mail
...
def init_db():
    with engine.begin() as conn:
        ...
        # Idempotent columns (no-ops if already there)
        for ddl in [
...
def send_email(subject: str, body: str):
    if not email_enabled():
        return False, "E-mail não configurado (defina SMTP_HOST, SMTP_USER, SMTP_PASS, EMAIL_FROM, EMAIL_TO)."
    try:
        msg = EmailMessage()
        msg["Subject"] = subject
        msg["From"] = EMAIL_FROM
...
# ====== UI & Auth ======
st.set_page_config(page_title="RNC — v2.6 (PDF + Logo + E-mail)", page_icon="🧭", layout="wide")
st.sidebar.title("RNC — v2.6")
st.sidebar.caption("RNC Nº automático • PDF • Logo • E-mail")
...
# Logo uploader (Qualidade)
with st.sidebar.expander("🖼️ Logo da empresa (para PDF e topo)"):
    logo_bytes = settings_get_logo()
    if logo_bytes:
        st.image(Image.open(io.BytesIO(logo_bytes)), caption="Logo atual", width=180)
...
if st.session_state.is_quality and st.sidebar.button("⚡ Testar envio de e-mail (auto-check)"):
    ok, msg = send_email("Teste de e-mail — RNC", "Este é um teste do app RNC v2.6.")
    st.sidebar.write("Resultado:", "OK" if ok else f"Falhou: {msg}")
...
"""
            ...
            if ok:
                st.info("E-mail de abertura enviado.")
            else:
                st.warning(f"E-mail não enviado: {msg} (configure SMTP_* e EMAIL_*).")
...
"""
                                ok, msg = send_email(subject, body)
                                if ok:
                                    st.info("E-mail de encerramento enviado.")
                                else:
                                    st.warning(f"E-mail não enviado: {msg}")
...
# ====== Exportar ======
    ...
    if df.empty:
        ...
        st.download_button("Baixar CSV", data=csv_bytes, file_name="rnc_export_v2_6.csv", mime="text/csv")
